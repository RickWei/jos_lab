...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
        ...
        if (i>=EXTPHYSMEM/PGSIZE&&(char*)page2kva(&pages[i])<first_free_page) {
            pages[i].pp_ref++;
            pages[i].pp_link = NULL;
            continue;
        }
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    if (*entry) {
        ...
        pp->pp_ref++;
        *entry=page2pa(pp)|PTE_P|PTE_W|PTE_U;
        pgtable=KADDR(PTE_ADDR(*entry));
    }
    else return NULL;
...
{
	// LAB 3: Your code here.
    uintptr_t end=(int)ROUNDUP(va+len,PGSIZE)-1;
    for (uintptr_t i=(int)ROUNDDOWN(va,PGSIZE); i<end; i+=PGSIZE) {
        pte_t* pte=pgdir_walk(env->env_pgdir,(void*)i,0);
        if ((!pte)||(!((*pte)&PTE_P))||(i>ULIM)||(((*pte)&perm)!=perm)) {
